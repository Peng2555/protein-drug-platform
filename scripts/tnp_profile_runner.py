...
def run_tnp_profile_job(
    *,
    ...
    _ensure_paths()
    from tnp_profile.workflow import run_workflow
    ...
    try:
        ...
        return TnpProfileResult(status="ok", stage="done", seconds=time.time() - t0, results=results)
        ...
        if wf.is_file():
            try:
                ...
                pass
        return TnpProfileResult(status="failed", stage=stage, seconds=time.time() - t0, error=str(exc)[:8000])
